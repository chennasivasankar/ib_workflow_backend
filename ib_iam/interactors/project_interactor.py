...
class ProjectInteractor(ValidationMixin):
    ...
    def get_user_team_dtos_for_given_project_teams_and_users_details_dto(
        ...
        valid_team_ids = self.project_storage.get_valid_team_ids(
    ...
    def _validate_team_existence_in_project(
            self, project_team_user_dto: ProjectTeamUserDTO):
        is_team_exists_in_project = self.project_storage.is_team_exists_in_project(
    ...
    def _validate_user_existence_in_given_team(
            self, project_team_user_dto: ProjectTeamUserDTO):
        is_user_exists_in_team = self.project_storage.is_user_exists_in_team(
            team_id=project_team_user_dto.team_id,
            user_id=project_team_user_dto.user_id
        )
    ...
    def _validate_user_ids_and_get_user_team_dtos_for_given_project(
        ...
        team_ids = self.project_storage.get_team_ids(project_id=project_id)
    ...
    def _convert_to_team_id_and_name_dto(
            user_team_dto: TeamWithUserIdDTO
    ) -> TeamIdAndNameDTO:
    ...
    def add_project(
        ...
        self._validate_add_project_details(
            project_with_team_ids_and_roles_dto=project_with_team_ids_and_roles_dto
        )
    ...
    def update_project(
        ...
        project_team_ids = self.project_storage.get_team_ids(
            project_id=complete_project_details_dto.project_id
        )
    ...
    def _assign_teams_to_project(
            self, project_id: str, project_team_ids: List[str],
            team_ids: List[str]
    ):
    ...
    def _remove_teams_from_project(
            self, project_id: str, project_team_ids: List[str],
            team_ids: List[str]
    ):
        team_ids_to_be_removed = list(set(project_team_ids) - set(team_ids))
        user_id_and_team_ids_dtos = self.project_storage.get_user_id_with_teams_ids_dtos(
            project_id=project_id
        )
        user_ids = [
    ...
    def _add_project_roles(self, project_id: str, roles: List[RoleDTO]):
        role_name_and_description_dtos = [
            RoleNameAndDescriptionDTO(
                name=role_dto.name, description=role_dto.description)
            for role_dto in roles if role_dto.role_id is None
        ]
        self.project_storage.add_project_roles(
            project_id=project_id, roles=role_name_and_description_dtos
        )

    def _update_project_roles(self, project_role_ids, roles):
        roles_to_be_updated = [
            role_dto
            for role_dto in roles if role_dto.role_id in project_role_ids
        ]
    ...
    def _validate_is_role_names_already_exists_for_add_project(
        ...
        role_names_that_already_exists = self.project_storage.get_valid_role_names(
            role_names=role_names
        )
    ...
    def _validate_is_given_name_already_exists_for_update_project(
        ...
        project_id_from_db = self.project_storage.get_project_id(name=name)
        if project_id_from_db not in [None, project_id]:
            raise ProjectNameAlreadyExists

    def _validate_is_given_display_id_already_exists(self, display_id: str):
        is_exists_display_id = self.project_storage.is_exists_display_id(
            display_id=display_id
        )
        if is_exists_display_id:
    ...
    def _validate_roles(
            self, roles: List[RoleDTO], project_role_ids: List[str]
    ):
        role_ids = [
            role_dto.role_id
    ...
    @staticmethod
    def _validate_invalid_role_ids(
            role_ids: List[str], project_role_ids: List[str]
    ):
