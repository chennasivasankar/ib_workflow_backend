...
class TeamInteractor(ValidationMixin):
    ...
    def update_team_details_wrapper(
        ...
        try:
            ...
            response = presenter.response_for_duplicate_user_ids_exception(err)
        except InvalidUserIds as err:
            response = presenter.response_for_invalid_user_ids_exception(err)
    ...
    def _add_members_to_team(
            self, user_ids: List[str], team_user_ids: List[str], team_id: str
    ):
    ...
    def _validate_is_team_name_already_exists(self, name: str):
        team_id = self.team_storage.get_team_id_if_team_name_already_exists(
            name=name
        )
    ...
    def _validate_is_team_name_exists_for_update_team(
            self, name: str, team_id: str
    ):
        team_id_from_db = self.team_storage.get_team_id_if_team_name_already_exists(
            name=name
        )
    ...
    def _get_invalid_team_ids(
            team_id_and_name_dtos: List[TeamIdAndNameDTO], team_ids: List[str]
    ) -> List[str]:
        valid_team_ids = [
            team_id_and_name_dto.team_id
