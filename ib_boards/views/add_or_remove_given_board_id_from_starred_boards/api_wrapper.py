...
def api_wrapper(*args, **kwargs):
    ...
    try:
        from ib_boards.views.add_or_remove_given_board_id_from_starred_boards.request_response_mocks \
    ...
    test_case = {
        "path_params": {'board_id': 'string'},
        "query_params": {},
        "header_params": {},
        "body": body,
        "securities": [{'oauth': ['read', 'write']}]
    }
    ...
    try:
        response = ''
        status_code = 200
        if '200' in ['403', '404', '200']:
            from ib_boards.views.add_or_remove_given_board_id_from_starred_boards.request_response_mocks \
                import RESPONSE_200_JSON
            response = RESPONSE_200_JSON
            status_code = 200
        elif '201' in ['403', '404', '200']:
            from ib_boards.views.add_or_remove_given_board_id_from_starred_boards.request_response_mocks \
