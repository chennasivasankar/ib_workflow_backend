...
class PopulateAddOrDeleteColumnsForBoard:
    ...
    def _convert_column_dict_to_column_dto(self, column_dict: Dict) -> ColumnDTO:
        ...
        return ColumnDTO(
            ...
            task_template_stages=column_dict['task_template_stages'],
            user_role_ids=column_dict['user_role_ids'],
            column_summary=column_dict['column_summary'],
            list_view_fields=column_dict['list_view_fields'],
            kanban_view_fields=column_dict['kanban_view_fields'],
    ...
    def validate_keys_in_given_dict(self, boards_columns_dicts: List[Dict]):
        ...
        schema = Schema(
            [{
                ...
                "board_display_name": And(str, len),
                "column_id": And(str, len),
                "column_display_name": And(str, len),
                "display_order": And(str, len),
                "user_role_ids": List[And(str, len)],
                Optional("column_summary"): str,
                Optional("column_actions"): str,
                "task_template_stages": json,
                "kanban_view_fields": json,
                "list_view_fields": json
    ...
    def _get_task_template_stages_dto(self, json_object):
        return 1

    def _get_task_template_summary_fields_dto(self, json_object):
        return 1
