...
USER_NEEDS_FILED_WRITABLE_PERMISSION = (
        "user needs write access on field {}, because user does not have at "
        "least one role in {} roles",
        "USER_NEEDS_FILED_WRITABLE_PERMISSION")
...
USER_IS_NOT_ASSIGNED_TO_TASK = (
        "user is not assigned to the task", "USER_IS_NOT_ASSIGNED_TO_TASK")

INVALID_DUE_DATE_TIME = (
        "given updated due datetime is invalid", "INVALID_DUE_DATE_TIME")
...
TASK_DELAY_REASON_NOT_UPDATED = (
        "Task delay reason not updated for task {} in stage {} with missed "
        "due "
        "date {}",
        "TASK_DELAY_REASON_NOT_UPDATED"
)
