...
class CreateFieldInteractor:
    def __init__(self, storage: CreateFieldStorageInterface):
        self.storage = storage

    def create_field(self, field_dtos: List[FieldDTO]):
        self._validate_field_id(field_dtos)
        self._check_for_duplication_of_filed_ids(field_dtos)
        self._check_for_duplication_of_drop_down_values_for_field_dtos(field_dtos)
    ...
    def _check_for_duplication_of_filed_ids(field_dtos):
        ...
        duplication_of_field_ids = [
            field_id for field_id, count in collections.Counter(field_ids).items() if count > 1
        ]
    ...
    def _check_for_duplication_of_drop_down_values_for_field_dtos(
        ...
        if fieds_with_dropdown_duplicate_values:
            raise FieldsDuplicationOfDropDownValues(fieds_with_dropdown_duplicate_values)
    ...
    def _check_for_duplication_of_dropdown_values(self, field_dto=FieldDTO):
        ...
        duplications_of_dropdown_values = [
            value for value, count in collections.Counter(dropdown_values).items() if count > 1
        ]
        if duplications_of_dropdown_values:
            fied_with_dropdown_duplicate_values["field_id"] = field_id
            fied_with_dropdown_duplicate_values["field_type"] = FieldTypes.DROPDOWN.value
            fied_with_dropdown_duplicate_values["duplicate_values"] = duplications_of_dropdown_values
    ...
    def _check_invalid_roles_for_read_permissions_in_field_dtos(
        ...
        for field_dto in field_dtos:
            ...
            if invalid_roles_dict:
                ...
                fields_invalid_roles_for_read_permission.append(invalid_roles_dict)
        if fields_invalid_roles_for_read_permission:
            raise InvalidRolesException(fields_invalid_roles_for_read_permission)
