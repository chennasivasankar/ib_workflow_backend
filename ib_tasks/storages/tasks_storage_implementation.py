...
class TasksStorageImplementation(TaskStorageInterface):
    ...
    def get_task_details(self, task_dtos: List[GetTaskDetailsDTO]) -> \
        ...
        task_objs = Task.objects.filter(id__in=task_ids).values('id', 'template_id')

        task_template_and_stage_ids = self._get_task_tempalate_and_stage_ids(task_dtos, task_objs)
        q = None
        for counter, item in enumerate(task_template_and_stage_ids):
            current_queue = Q(task_template_id=item['template_id'], stage_id=item['stage_id'])
            if counter == 0:
                q = current_queue
            else:
                q = q | current_queue
    ...
    def _get_stage_action_objs(task_template_and_stage_ids):
        q = None
        for counter, item in enumerate(task_template_and_stage_ids):
            current_queue = Q(stage__task_template_id=item['template_id'], stage__stage_id=item['stage_id'])
            if counter == 0:
                q = current_queue
            else:
                q = q | current_queue
        stage_actions = StageAction.objects.filter(q).values('id', 'stage__stage_id', 'name', 'button_text',
                                                             'button_color')
        ...
        for stage in stage_objs:
            for field in field_objs:
                if field['field_id'] in stage['field_display_config']:
                    ...
                    fields_dtos.append(
                        self.get_field_dto(field, field_id, field_values, stage)
                    )
        return fields_dtos
