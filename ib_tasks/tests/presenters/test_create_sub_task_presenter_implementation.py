...
class TestCreateSubTaskPresenterImplementation:

    @pytest.fixture
    def presenter(self):
        from ib_tasks.presenters.create_sub_task_presenter_implementation \
            import CreateSubTaskPresenterImplementation
        presenter = CreateSubTaskPresenterImplementation()
        return presenter

    def test_raise_invalid_parent_task_id(self, snapshot, presenter):
        # Arrange
        given_parent_task_id = "IBWF-1"
        from ib_tasks.exceptions.task_custom_exceptions import \
            InvalidTaskDisplayId
        err = InvalidTaskDisplayId(given_parent_task_id)

        # Act
        response_object = presenter.raise_invalid_parent_task_id(err)
    ...
    def test_get_create_sub_task_response(self, snapshot, presenter):
        # Arrange
        ...
        complete_task_details_dto = CompleteTaskDetailsDTO(
            task_id=1, created_task_display_id=created_task_display_id,
            task_current_stages_details_dto=task_current_stage_details_dto,
            all_tasks_overview_details_dto=all_tasks_overview_dto)

        # Act
        response_object = presenter.get_create_sub_task_response(
            complete_task_details_dto)

        # Assert
        response = json.loads(response_object.content)
        snapshot.assert_match(response, 'create_sub_task_response')
