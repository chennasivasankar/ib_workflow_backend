...
class TestGetActionsInteractor(StorageMethodsMock):
    ...
    def get_actions_dtos(self):
        ...
        actions.append(StageActionDetailsDTOFactory(
                stage_id="stage_id_2",
                action_type=ActionTypes.NO_VALIDATIONS.value))
        actions.append(StageActionDetailsDTOFactory(
                stage_id="stage_id_2",
    ...
    def test_when_task_ids_is_invalid_raises_exception(self,
        ...
        interactor = GetTaskActionsInteractor(action_storage,
                                              task_storage,
                                              stage_storage)
    ...
    def test_when_stage_ids_is_invalid_raises_exception(self,
        ...
        user_roles = get_user_roles_mock
        user_roles_mock = get_user_role_ids_based_on_projects_mock(mocker)
        stage_storage.get_existing_stage_ids.return_values = [
                "stage_id_1"]
        user_roles_mock.return_value = user_roles
        task_storage.get_valid_task_ids.return_value = [1, 2]
        action_storage.get_actions_details.return_value = action_dtos
        interactor = GetTaskActionsInteractor(action_storage,
                                              task_storage,
                                              stage_storage)
