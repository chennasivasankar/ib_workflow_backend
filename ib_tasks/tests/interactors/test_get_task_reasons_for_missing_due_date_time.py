...
class TestGetTaskReasons(StorageMockClass):
    ...
    def test_given_invalid_task_id_raises_exception(self, stage_storage):
        # Arrange
        task_id = "iBWF-1"
        user_id = "user_id_1"
        stage_id = 1
        ...
        interactor = GetTaskDueMissingReasonsInteractor(
            storage=storage, task_storage=task_storage, stage_storage=stage_storage
        )
    ...
    def test_given_invalid_stage_id_raises_exception(self, stage_storage):
        # Arrange
        task_display_id = "iBWF-1"
        task_id = 1
        user_id = "user_id_1"
        ...
        task_storage.check_is_valid_task_display_id.return_value = True
        task_storage.get_task_id_for_task_display_id.return_value = 1
        stage_storage.validate_stage_id.return_value = False
        ...
        # Assert
        stage_storage.validate_stage_id.assert_called_once_with(
            stage_id)
        presenter.response_for_invalid_stage_id.assert_called_once()

    def test_given_task_due_missing_details(self,
                                            mocker,
        ...
        task_storage.get_task_due_details.return_value = \
            get_due_missing_details
    ...
    def test_given_valid_details_get_task_due_missing_details(
            self, mocker, get_due_missing_details, stage_storage):
        # Arrange
        task_display_id = "iBWF-1"
        task_id = 1
        user_id = "user_id_1"
        ...
        stage_storage.validate_stage_id.return_value = True
        task_storage.check_is_valid_task_display_id.return_value = True
        task_storage.get_task_id_for_task_display_id.return_value = 1
        # task_storage.validate_if_task_is_assigned_to_user_in_given_stage.return_value = True
        task_storage.get_task_due_details.return_value = \
            get_due_missing_details
        presenter.get_response_for_get_task_due_details.return_value = expected_response
        assignee_details_dtos = assignee_details_dtos_mock(mocker)
