"""
test with user who does not have permission to perform the given action
"""
...
from ...factories.models import (
    ...
    CurrentTaskStageModelFactory, ProjectTaskTemplateFactory)


class TestCase44SaveAndActOnATaskAPITestCase(TestUtils):
    APP_NAME = APP_NAME
    ...
    def setup(self, mocker):
        ...
        ProjectTaskTemplateFactory.create(task_template=template,
                                          project_id=project_id)
        ...
        action_stage = StageFactory(
            id=3, task_template_id=template.template_id)
        CurrentTaskStageModelFactory.create(task=task, stage=action_stage)
        ...
        StagePermittedRolesFactory.create(stage=action_stage,
                                          role_id="FIN_PAYMENT_REQUESTER")
