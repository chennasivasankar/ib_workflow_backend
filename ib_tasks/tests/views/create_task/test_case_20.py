"""
test with invalid number for number field raises exception
"""
import datetime
...
class TestCase20CreateTaskAPITestCase(TestUtils):
    APP_NAME = APP_NAME
    ...
    def setup(self, mocker):
        ...
        field_obj = FieldFactory.create(
            gof=gof_obj, field_type=FieldTypes.NUMBER.value)
    ...
    @pytest.mark.parametrize('number_value', ['iB', 700.0])
    @freezegun.freeze_time(datetime.datetime(2020, 8, 31, 5, 4, 54))
    def test_case(self, snapshot, number_value):
        body = {
            "project_id": "project_1",
            "task_template_id": "template_1",
            "action_id": 1,
