"""
test with valid details creates transition checklist
when there are some existing fields and new fields too..
then, this api should create new fields and update existing fields
"""
import pytest
from django_swagger_utils.utils.test_utils import TestUtils

from . import APP_NAME, OPERATION_NAME, REQUEST_METHOD, URL_SUFFIX
from ...factories.models import TaskGoFFactory, TaskGoFFieldFactory


class TestCase34CreateTransitionChecklistAPITestCase(TestUtils):
    APP_NAME = APP_NAME
    ...
    def setup(self, mocker):
        ...
        gof_id = "gof_1"
        field_id = "field_1"
        existing_gof_id = "FIN_REQUESTOR_DETAILS"
        existing_field_id = "FIN_TEAM_NAME"
        ...
        existing_gof_obj = GoFFactory.create(gof_id=existing_gof_id)
        existing_field_obj = FieldFactory.create(
            field_id=existing_field_id, gof=existing_gof_obj)
        existing_task_gof_obj = TaskGoFFactory.create(
            task=task_obj, gof=existing_gof_obj)
        TaskGoFFieldFactory.create(
            task_gof=existing_task_gof_obj, field=existing_field_obj,
            field_response="Alpha Team")
        GoFToTaskTemplateFactory.create(
            gof=gof_obj, task_template=transition_template_obj)
        GoFToTaskTemplateFactory.create(
            gof=existing_gof_obj, task_template=transition_template_obj)
        ...
        FieldRoleFactory.create(
            field=existing_field_obj, role="FIN_PAYMENT_REQUESTER",
            permission_type=PermissionTypes.WRITE.value
        )
                    ...
                    'gof_fields': [
                        {
                            ...
                            'field_response': "Beta Team - updated"
                        }
                    ]
                }
            ]
