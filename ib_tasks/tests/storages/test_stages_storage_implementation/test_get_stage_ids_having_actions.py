...
from ib_tasks.tests.factories.models import StagePermittedRolesFactory, \
    ActionPermittedRolesFactory, StageModelFactory, \
    StageActionFactory
...
class TestGetStageIdsHavingActions:
    ...
    def setup(self):
        ...
        ActionPermittedRolesFactory(action=actions[0],
                                    role_id='FIN_PAYMENT_REQUESTER')
        ActionPermittedRolesFactory(action=actions[1],
                                    role_id='FIN_PAYMENT_APPROVER')
        ActionPermittedRolesFactory(action=actions[2],
                                    role_id='FIN_PAYMENT_REQUESTER')
        ActionPermittedRolesFactory(action=actions[3],
                                    role_id='FIN_PAYMENT_APPROVER')
