...
class TestUserTaskWithRecentStageDetails:
    ...
    def create_task_stages_setup(self, create_task):
        ...
        TaskStageHistoryModelFactory.create_batch(size=3, task=task_obj)

    @pytest.fixture()
    def stages_having_assignees_dtos(self):
        stages_having_assignees_dtos = \
            TaskStageHavingAssigneeIdDTOFactory.create_batch(
            2)
        return stages_having_assignees_dtos

    def test_get_stage_details_having_assignees_in_given_stage_ids(
            self, create_task, create_task_stages_setup,
            stages_having_assignees_dtos):
        # Arrange
        task_obj = create_task
        stage_ids = [1, 2]
