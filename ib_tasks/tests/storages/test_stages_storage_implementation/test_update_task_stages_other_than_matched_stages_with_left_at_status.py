...
class TestUpdateTaskStagesWithLeftAtStatus:
    ...
    def populate_task_stage_histories(self):
        ...
        stage_histories_2 = TaskStageHistoryModelFactory.create_batch(
            2, task=task)
        return
    ...
    def test_update_task_stages_other_than_matched_stages_with_left_at_status(
        ...
        # Assert
        task_stage_objects = list(TaskStageHistory.objects.filter(
            task_id=task_id).values('task_id', 'stage_id', 'assignee_id',
                                    'left_at'))

        snapshot.assert_match(task_stage_objects, "task_stage_objects")
